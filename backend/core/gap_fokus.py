...
PETUNJUK = [
    ...
    ("  2. Sheet BOM_AKSESORIS — kelompok bahan yang belum bisa masuk. Kolom paling kanan 'yang_perlu_diisi' menyebut persis apa yang kurang di baris itu.", False),
...
async def build_fokus_workbook(db, parsed: dict | None, data: bytes | None) -> tuple[bytes, dict]:
    ...
    if parsed:
        ...
        for r, st in zip(b_rows, b_styles):
            ws.append(r)
            i = ws.max_row
            for col, fill in st["fills"]:
                ws.cell(row=i, column=BOM_FOKUS_COLS.index(col) + 1).fill = fill
    ...
    _widths(ws, (12, 22, 16, 40, 12, 8, 30, 26, 60, 80))
    ws.freeze_panes = "C2"
